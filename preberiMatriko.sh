...
function multiply(){ #funkcija za mnozenje
	...
	then
		...
		for(( x=0; x<$(($vrstice1 * $stolpci1)); x+=stolpci1 )) #ker bash nima vecdimenzionalnih polj
		done 													#prilagodimo for zanke tako da gre prva vrtice*stolpci prve matrike
			for(( y=0; y<$stolpci2; y++ )) #sprehodimo se cez stolpec druge matrike
			do
				for (( i = 0, t=0; i < $stolpci1; i++, t+=$vrstice2 )) #i se sprehodi cez stolpce prve matrike,
				do 													#t pa se povecuje za toliko kot je vrtic v 2 matriki saj potrebujemo naslednjo spodnjo vrednost druge matrike pri mnozenju
					rezultat[$stevec]=$(awk -v prva="${polje1[$(($x + $i))]}" -v druga="${polje2[$(($t + $y))]}" -v rez="${rezultat[$stevec]}" 'BEGIN{print (rez+(prva*druga))}')
				done  #rezultat zapisemo z ukazom awk, naredimo mnozenje polja1[x+i]*polje2[t+y] + trenutna vrednost rezultata
				stevec=$[$stevec+1]
			done
		done
		echo
